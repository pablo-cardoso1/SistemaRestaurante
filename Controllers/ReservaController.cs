...
public class ReservaController : ControllerBase
{
    ...
    [HttpGet]
    [Authorize]
    public async Task<ActionResult<IEnumerable<ReservaRespostaDTO>>> ListarReservas()
    {
        var usuarioId = int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier));
        var reservas = await _context.Reservas
            .Where(r => r.UsuarioId == usuarioId)
            .Select(r => new ReservaRespostaDTO
            {
                Id = r.Id,
                MesaId = r.MesaId,
    ...
    {
        var usuarioId = int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier));
        var reserva = await _context.Reservas.Include(r => r.Mesa).FirstOrDefaultAsync(r => r.Id == id);
        if (reserva == null)
            return NotFound();
        if (reserva.UsuarioId != usuarioId)
            return Forbid();
        if (reserva.Status != "ativo")
            return BadRequest("Reserva já está cancelada.");

        reserva.Status = "cancelado";
        // Libera a mesa se não houver outra reserva ativa para ela
        var existeOutraAtiva = await _context.Reservas.AnyAsync(r => r.MesaId == reserva.MesaId && r.Status == "ativo" && r.Id != reserva.Id);
        if (!existeOutraAtiva && reserva.Mesa != null)
